...
typedef uint8_t u8;
#define SKIPJACK_KEYSIZE_B 10 /* key size in bytes */	
#define SKIPJACK_BLOCK_SIZE_B 8 /* Block size in bytes */

typedef struct {
	u8 key_tables[SKIPJACK_KEYSIZE_B][256];
	//u8 output[SKIPJACK_BLOCK_SIZE_B];
} Skipjack_CTX;

void Skipjack_init( Skipjack_CTX *ctx, u8 *key);
void Encrypt( Skipjack_CTX *ctx, u8 *in, u8 *out);
void Decrypt( Skipjack_CTX *ctx, u8 *in, u8 *out);
